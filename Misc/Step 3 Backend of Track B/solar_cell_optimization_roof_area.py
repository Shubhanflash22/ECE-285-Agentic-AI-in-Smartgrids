"""
recommend_energy_system.py  (v4 — single file)
===============================================
...
def _load_household_profile_from_eia(latitude: float, longitude: float,
    ...
    f4 = (np.clip(rng_hh.normal(1.0, 0.15), 0.7, 1.3)
          * np.clip(rng_hh.normal(1.0, 0.10), 0.8, 1.2))
    ...
    ev_prob = (0.30 if is_affluent
               else 0.15 if (is_coastal or (longitude > -117.05 and latitude > 32.75))
               else 0.10 if is_urban_core else 0.08)
    if rng_ev.random() < ev_prob:
        hrs      = np.arange(8760) % 24
        start_h  = rng_ev.randint(18, 24)
        end_h    = (start_h + rng_ev.randint(3, 7)) % 24
        is_chg   = ((hrs >= start_h) | (hrs < end_h)) if start_h >= end_h \
                   else ((hrs >= start_h) & (hrs < end_h))
        ckw      = rng_ev.uniform(3.0, 7.0)
        profile += np.where(is_chg, ckw, 0.0)
        print(f"    [EIA] EV adopted     ({ckw:.1f} kW, {start_h}:00–{end_h}:00)")
...
def recommend_residential_energy_system(user_profile: dict) -> dict:
    ...
    try:
        load, akwh = _load_household_profile_from_eia(lat, lon, akwh)
    ...
    def acfg(n):
        """Array config: series×parallel strings targeting ~400 V."""
        bs2, bp2, bd = 1, n, 1e9
        for ns2 in range(1, n + 1):
            if n % ns2 != 0: continue
            diff = abs(ns2 * panel["vmp_v"] - 400)
            if diff < bd: bd = diff; bs2 = ns2; bp2 = n // ns2
        return {"panels_in_series_per_string":   bs2,
                "parallel_strings":              bp2,
                "total_panels":                  n,
                "array_voltage_v":               round(bs2 * panel["vmp_v"], 2),
                "array_current_a":               round(bp2 * panel["imp_a"], 2),
...
SOLAR_PANEL_CATALOG = [
    {"manufacturer": "REC Group",      "model": "Alpha Pure",
     "efficiency_percent": 22.6, "cost_per_wp_usd": 2.85, "temp_coeff_pct_per_C": -0.26,
     "voc_v": 43.2, "isc_a": 13.8, "vmp_v": 36.4, "imp_a": 13.1,
     "panel_power_w": 405, "panel_area_m2": 1.79,
     "cells_in_series": 60, "cells_in_parallel": 1, "degradation_rate": 0.005},
    {"manufacturer": "JA Solar",       "model": "DeepBlue",
     "efficiency_percent": 21.5, "cost_per_wp_usd": 2.90, "temp_coeff_pct_per_C": -0.35,
     "voc_v": 41.8, "isc_a": 13.5, "vmp_v": 34.9, "imp_a": 12.7,
     "panel_power_w": 395, "panel_area_m2": 1.84,
     "cells_in_series": 60, "cells_in_parallel": 1, "degradation_rate": 0.006},
    {"manufacturer": "Trina Solar",    "model": "Vertex S",
     "efficiency_percent": 21.8, "cost_per_wp_usd": 2.90, "temp_coeff_pct_per_C": -0.34,
     "voc_v": 42.5, "isc_a": 13.6, "vmp_v": 35.5, "imp_a": 12.9,
     "panel_power_w": 400, "panel_area_m2": 1.83,
     "cells_in_series": 60, "cells_in_parallel": 1, "degradation_rate": 0.006},
    {"manufacturer": "Canadian Solar", "model": "TOPHiKu7",
     "efficiency_percent": 22.5, "cost_per_wp_usd": 3.10, "temp_coeff_pct_per_C": -0.30,
     "voc_v": 44.0, "isc_a": 14.0, "vmp_v": 37.0, "imp_a": 13.3,
     "panel_power_w": 420, "panel_area_m2": 1.87,
     "cells_in_series": 66, "cells_in_parallel": 1, "degradation_rate": 0.005},
    {"manufacturer": "Silfab Solar",   "model": "Prime",
     "efficiency_percent": 22.1, "cost_per_wp_usd": 3.15, "temp_coeff_pct_per_C": -0.33,
     "voc_v": 43.5, "isc_a": 13.7, "vmp_v": 36.0, "imp_a": 13.0,
     "panel_power_w": 410, "panel_area_m2": 1.85,
     "cells_in_series": 60, "cells_in_parallel": 1, "degradation_rate": 0.005},
    {"manufacturer": "Jinko Solar",    "model": "Tiger Neo",
     "efficiency_percent": 23.8, "cost_per_wp_usd": 3.20, "temp_coeff_pct_per_C": -0.29,
     "voc_v": 45.1, "isc_a": 14.2, "vmp_v": 38.0, "imp_a": 13.6,
     "panel_power_w": 440, "panel_area_m2": 1.85,
     "cells_in_series": 66, "cells_in_parallel": 1, "degradation_rate": 0.005},
    {"manufacturer": "LONGi Solar",    "model": "Hi-MO 6",
     "efficiency_percent": 23.3, "cost_per_wp_usd": 3.35, "temp_coeff_pct_per_C": -0.29,
     "voc_v": 44.8, "isc_a": 14.1, "vmp_v": 37.7, "imp_a": 13.5,
     "panel_power_w": 435, "panel_area_m2": 1.87,
     "cells_in_series": 66, "cells_in_parallel": 1, "degradation_rate": 0.005},
    {"manufacturer": "Maxeon Solar",   "model": "Maxeon 7",
     "efficiency_percent": 22.8, "cost_per_wp_usd": 3.50, "temp_coeff_pct_per_C": -0.27,
     "voc_v": 44.5, "isc_a": 13.9, "vmp_v": 37.3, "imp_a": 13.2,
     "panel_power_w": 430, "panel_area_m2": 1.89,
     "cells_in_series": 66, "cells_in_parallel": 1, "degradation_rate": 0.004},
    {"manufacturer": "Aiko Solar",     "model": "Neostar 2P",
     "efficiency_percent": 24.3, "cost_per_wp_usd": 3.75, "temp_coeff_pct_per_C": -0.24,
     "voc_v": 46.0, "isc_a": 14.5, "vmp_v": 38.8, "imp_a": 13.9,
     "panel_power_w": 460, "panel_area_m2": 1.89,
     "cells_in_series": 66, "cells_in_parallel": 1, "degradation_rate": 0.004},
]
...
BATTERY_CATALOG = [
    {"manufacturer": "Tesla",     "model": "Powerwall 3",
     "usable_capacity_kwh": 13.5, "nominal_voltage_v": 50.0,
     "max_charge_power_kw": 11.5, "max_discharge_power_kw": 11.5,
     "round_trip_efficiency_pct": 97.5, "cycle_life": 4000,
     "cost_usd": 11500, "annual_degradation_rate": 0.010},
    {"manufacturer": "Enphase",   "model": "IQ Battery 5P",
     "usable_capacity_kwh": 5.0,  "nominal_voltage_v": 48.0,
     "max_charge_power_kw": 3.84, "max_discharge_power_kw": 3.84,
     "round_trip_efficiency_pct": 96.0, "cycle_life": 4000,
     "cost_usd": 6000, "annual_degradation_rate": 0.012},
    {"manufacturer": "Generac",   "model": "PWRcell M6",
     "usable_capacity_kwh": 9.0,  "nominal_voltage_v": 48.0,
     "max_charge_power_kw": 6.7,  "max_discharge_power_kw": 6.7,
     "round_trip_efficiency_pct": 96.5, "cycle_life": 3500,
     "cost_usd": 10000, "annual_degradation_rate": 0.012},
    {"manufacturer": "SolarEdge", "model": "Home Battery 48V",
     "usable_capacity_kwh": 9.7,  "nominal_voltage_v": 48.0,
     "max_charge_power_kw": 5.0,  "max_discharge_power_kw": 5.0,
     "round_trip_efficiency_pct": 94.5, "cycle_life": 6000,
     "cost_usd": 9500, "annual_degradation_rate": 0.010},
    {"manufacturer": "Panasonic", "model": "EverVolt H Series",
     "usable_capacity_kwh": 17.1, "nominal_voltage_v": 51.8,
     "max_charge_power_kw": 7.6,  "max_discharge_power_kw": 7.6,
     "round_trip_efficiency_pct": 97.0, "cycle_life": 6000,
     "cost_usd": 15000, "annual_degradation_rate": 0.010},
]    